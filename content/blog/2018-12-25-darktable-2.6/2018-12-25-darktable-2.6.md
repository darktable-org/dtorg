...
These curves allow to control the force of denoising frequency by frequency.
In other words, you can adapt the force of the denoising to the noise coarseness.
The "all" curve allow to change the force for all channels at the same time, while "R", "G" and "B" curves allow to change the force separately for red, green, and blue channels.

Let's see an example of what can be done with the "all" curves first.
First, I zoomed into the image to be at a 100% zoom level.
...
I usually start with a force between 0.150 and 0.3.
Here, to better see the influence of the curve, I used a force of 0.5.
...
Here, we obtain an image which is much less disturbing as the last one: the fine grain noise is less distrurbing than the coarse grain one.
This image is even more natural (in my point of view) than the one with a flat curve.
...
* one for chroma noise (false colors), where the denoising is increased for fine details, as the color should not change much form one pixel to another.
* one for luma noise (false luminance), where the denoising is reduced on finest details and some coarse scales. It aims at providing a nice trade-off between noise and smoothing for picture that are not too much noisy (forget about extended ISO values for instance. For such pictures we have to use less automatic strategies).
...
Note that denoising RGB channels should be done BEFORE using an instance in color blend mode, as doing this will mix the channels and change the characteristics of noise.

These explanations were all using the denoise profiled module as an example, but you can follow the same steps with the rawdenoise module.
Also note that the tip of denoising RGB channels separately visualizing the channels using the channel mixer is also useful to set the parameters of the denoise bilateral module.
