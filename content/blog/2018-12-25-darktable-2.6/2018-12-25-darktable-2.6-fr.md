...
dans des éditeurs orientés pixels tel que GIMP, comme le module "liquéfier". 
...
ss(![outil de correction](rt-heal-icon.png)) donne ceci :
...
Les outils peuvent être activés une fois pour plusieurs corrections.
Utilisez simplement la combinaison Control-clic sur l'un des outils 
point, ligne, ellipse ou chemin (au lieu d'un simple clic), et l'outil 
restera actif jusqu'à ce que vous le désactiviez explicitement. Ceci 
est très pratique lorsque vous avez à corriger plusieurs endroits de la 
...
a [pixls.us PlayRaw contest](https://discuss.pixls.us/t/playraw-hillbilly-portrait/5518):
