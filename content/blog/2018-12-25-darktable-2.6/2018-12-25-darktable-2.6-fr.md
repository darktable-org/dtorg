...
vous cliquez simplement sur la partie de l'image que vous souhaitez effacer. Le module va cloner
...
#### Seamless cloning, de meilleurs résultats en moins d'efforts

Par défaut, le clone utilises un algorithme "seamless cloning" 
(repris de l'outil "heal de GIMP), qui adaptes la source au 
contexte de l'emplacement cloné.Vous n'avez pas besoin de cloner 
*exactement* la meilleure partie de l'image. Prenons un exemple 
classique : un petit défaut dans un ciel pas complètement uniforme :

![exemple de retouche](rt-original.png)

Une mauvaise tentative pour fixer cela avec l'outil clone donnerait :

![clone de retouche (avec contrôles)](rt-clone.png)
...
La même retouche  avec le nouvel outil "heal"
(![heal tool](rt-heal-icon.png)) donne ceci :

![Retouch heal (with controls)](rt-heal.png)

Cette fois, l'image finale is undistinguishable from a real sky
without defect:
