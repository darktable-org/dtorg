...
Par défaut, le clone utilises un algorithme "seamless cloning"  (borrowed
from GIMP's "heal" tool), qui adaptes la source to the context
where it is cloned. You don't need to clone *exactly* the right
piece of image. Prenons un exemple classique : un petit défaut dans un 
ciel pas complètement uniforme :

![exemple de retouche]({filename}rt-original.png)

A bad attempt to fix this with the clone tool would give:

![clone de retouche (avec contrôles)]({filename}rt-clone.png)
